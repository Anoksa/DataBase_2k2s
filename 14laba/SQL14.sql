...
declare @faculty int =dbo.COUNT_STUDENTS('ПиМ');
...
select distinct faculty,profession, dbo.COUNT_STUDENTS(faculty)
			from groups 
			where dbo.COUNT_STUDENTS(faculty )<>0--???
...
--6	
	...
	create function FACULTY_REPORT1(@c int) returns @fr table
		...
		as begin 
	       ...
	       while @@fetch_status = 0
	       begin
	            insert @fr values( @f,  dbo.count_pulpit(@f),dbo.count_group(@f),   dbo.COUNT_STUDENTS(@f),
	...
	go
	create function Count_Books(@c int) returns int
	as begin declare @rc int=0;
	set @rc = (select count(Книги.Название) from Книги
					where Книги.[Год  издания] = @c);
	return @rc;
	end;

	declare @c  int = dbo.Count_Books(2019);
	print 'Количество книг=' + cast(@c as varchar(4));

	
	
	create function Books(@f varchar(50), @p int)
	returns table
	as return
	select Книги.Название, Книги.Цена 
		from Книги
		where Книги.Название = ISNULL(@f, Книги.Название)
			and Книги.Цена = ISNULL(@p, Книги.Цена);

	select * from dbo.Books(Null, Null)

	select * From dbo.Books('Элита', Null)

	Select * From dbo.Books(Null, 9)