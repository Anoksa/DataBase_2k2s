...
AS
UPDATE Книги SET Цена=Цена*(1-@p)
  WHERE Название=@t

EXEC my_proc1 @t='Элита', @p=0.05
...
as begin
DECLARE @n int = (SELECT count(*) from Книги);
SELECT [id книги] [КОД], Название [Название], Цена [Стоимость] from Книги;
return @n;
end;

DECLARE @k int;
EXEC @k = my_proc3; -- вызов процедуры 
print 'Количество книг: ' + cast(@k as varchar(3));
go